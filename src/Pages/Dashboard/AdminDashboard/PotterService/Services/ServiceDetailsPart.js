...
//   import { alert, ButtonStyle } from "../../Hooks/useStyle";
  ...
  const ServiceDetailsPart = (props) => {
    // const {details}=props.store;
    const { stores } = props;
...
const manageservice = service?.filter(model => model?._id
    === id);
      ...
      <>
        ...
        <Container>
          <Box sx={{ textAlign: "center", my: 5 }}>
            <Typography variant="h4" style={{fontWeight:"600"}}>Product Name :gg {manageservice.categories}</Typography>
            <Typography variant="h5" style={{fontWeight:"600"}}> Product Price : TK.{manageservice?.ProductPrice}</Typography>
          </Box>
...
{review?.length > 0 ? (
  ...
  export default ServiceDetailsPart;